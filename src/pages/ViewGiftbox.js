...
function Contactus() {
  ...
  useEffect(() => {
    ...
    const fetchProperties = async () => {
      try {
        const response = await axios.get(`${apiUrl}Allcustom_box`);
    ...
    const handleDelete = async (propertyId) => {
          ...
          if (result.isConfirmed) {
            try {
              ...
              await axios.delete(`${apiUrl}Allboxs/${propertyId}`);
              ...
              <div className="container-fluid">
                <div className="mb-4">
                  <div className='propertys-div'>
                    <h1 className="h3 mb-0 text-white text-center pt-3">Gift Boxs</h1>
  ...
  <table id="example" className="table table-striped table-bordered table-responsive">
    <thead>
      <tr>
        ...
        <th className="bl5 text-nowrap" style={{ width: '200px' }}>ITEMS</th>
    ...
    <tbody>
      {properties.map((property) => (
        <tr key={property.id}>
          ...
          <td className="text-nowrap">{property.box_price}</td>
          <td className="text-nowrap">{property.mrp}</td>
          <td className="text-nowrap">{property.space}</td>
          <td className="text-nowrap">
          
                        {/* Parse property.img as an array and map over it */}
            {JSON.parse(property.img).map((url, index) => (
                <img 
                    key={index} 
                    src={url} 
                    alt={`Image ${index + 1}`} 
