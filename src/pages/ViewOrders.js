...
doc.text(placeS, leftMargin, invoiceY + 20); // Left aligned Invoice Date
      ...
      doc.autoTable({
        startY: tableStartY,
        head: [['Order NO', 'Shipping','Tax (18%)', 'Base Amount','Total Amount']],
        body: [[
          property.orderId,
          property.paymentMethod === 'COD' ? '₹30' : 'FREE', // Conditional expression for shipping cost
      ...
      if (property.items && typeof property.items === 'string') {
        ...
        doc.autoTable({
          ...
          body: itemsData.map(item => {
            const price = parseFloat(item[4].replace('₹', '')); // Get price as a number
